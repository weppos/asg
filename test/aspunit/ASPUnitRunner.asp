		...
		If Not(oTestResult Is Nothing) Then
			...
			If (Request.Form("chkShowSuccess") <> "") Then
	            For Each oResult in oTestResult.Successes
					Response.Write("	<TR CLASS=""success""><TD>Success</TD><TD>" & TestName(oResult) & "</TD><TD>" & oResult.Source & oResult.Description & "</TD></TR>")
	            Next
	        End If

			For Each oResult In oTestResult.Errors
				Response.Write("	<TR CLASS=""error""><TD>Error</TD><TD>" & TestName(oResult) & "</TD><TD>" & oResult.Source & " (" & Trim(oResult.ErrNumber) & "): " & oResult.Description & "</TD></TR>")
			Next

			For Each oResult In oTestResult.Failures
				Response.Write("	<TR CLASS=""warning""><TD>Failure</TD><TD>" & TestName(oResult) & "</TD><TD>" & oResult.Description & "</TD></TR>")
			Next

			Response.Write("	<TR><TD ALIGN=""center"" COLSPAN=3>" & "Tests: " & oTestResult.RunTests & ", Errors: " & oTestResult.Errors.Count & ", Failures: " & oTestResult.Failures.Count & "</TD></TR>")
		End If
%>
			</TABLE>
<%
