...
if ASG_USE_MYSQL then
	strAsgSQL = strAsgSQL & " GROUP BY system_browser " &_
...
<div align="center">
	<div id="layout">
		<div id="layout_menutitle"><%= "<span class=""menusubtitle"">" & MENUGROUP_Visitors & " &raquo; " & MENUSECTION_VisitorSystems & " &raquo;</span> " & MENUSECTION_Browsers %></div>
		<div id="layout_content">

<%

' :: Open tlayout :: MENUSECTION_Browsers
Response.Write(builTableTlayout("", "open", MENUSECTION_Browsers))

%>
<table width="100%" border="0" align="center" cellpadding="1" cellspacing="1">
  <tr>
	<td width="5%"  class="treport_title">&nbsp;</td>
	<td width="33%" class="treport_title"><%= TXT_browser %>
	<% strOrderBy_message = Replace(TXT_orderBy_schema, "$field$", TXT_browser) %>
...
strAsgSQL = "SELECT system_browser, SUM(system_hits) AS SumHits, SUM(system_visits) As SumVisits " &_
	"FROM " & ASG_TABLE_PREFIX & "system "
' Month and Year
if strAsgMode = "month" then 
...
strAsgSQL = strAsgSQL & "GROUP BY system_browser "
...
%>		  
  <tr <%= buildTableContRollover("treport_row") %> >
	<td class="treport_col" style="text-align: center;"><img src="<%= STR_ASG_SKIN_PATH_IMAGE %>def/browser.asp?icon=<%= strAsgReportBrowser %>" alt="<%= strAsgReportBrowser %>" /></td>
	<td class="treport_col" style="text-align: left;"><% Response.Write(showIconTipUnknows(strAsgReportBrowser)) : Response.Write(searchTerms(strAsgReportBrowser, "system_browser", asgSearchfor, asgSearchIn)) %></td>
...
Response.Write(builTableTlayout("rowNavy", "open", buildSwapDisplay("rowNavy", BARLABEL_DataView)))
	...
	'Call buildTableContCheckIcon(4, "browser", page)
...
Response.Write(builTableTlayout("x-rowSearch", "open", buildSwapDisplay("rowSearch", BARLABEL_DataSearch)))
	...
	' Row - Layers search
	Response.Write(buildLayerSearch("", "system_browser"))
