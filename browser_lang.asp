...
if strAsgMode = "month" then 
	strAsgSQL = "SELECT Sum(lang_hits) As SumHits, Sum(lang_visits) AS SumVisits " &_
		"FROM " & ASG_TABLE_PREFIX & "language WHERE lang_period = '" & strAsgPeriod & "' "
elseif strAsgMode = "all" then 
	strAsgSQL = "SELECT Sum(lang_hits) As SumHits, Sum(lang_visits) AS SumVisits " &_
		"FROM " & ASG_TABLE_PREFIX & "language "
end if
' Open Rs
...
if strAsgMode = "month" then 
	strAsgSQL = "SELECT SUM(lang_hits) AS SumHits " &_
		"FROM " & ASG_TABLE_PREFIX & "language " &_
		"WHERE lang_period = '" & strAsgPeriod & "' "
elseif strAsgMode = "all" then 
	strAsgSQL = "SELECT SUM(lang_hits) AS SumHits " &_
		"FROM " & ASG_TABLE_PREFIX & "language "
end if
...
If ASG_USE_MYSQL then
	strAsgSQL = strAsgSQL & " GROUP BY lang_name " &_
		"ORDER BY SumHits DESC"
else
	strAsgSQL = strAsgSQL & " GROUP BY lang_name " &_
		"ORDER BY SUM(lang_hits) DESC"
end if
' Set Rs properties
if ASG_USE_MYSQL then
...
Select Case strAsgSortBy
	Case "hits" 	strAsgSortByFld = formatSortingField("SumHits", "SUM(lang_hits)", ASG_USE_MYSQL)
	Case "visits"	strAsgSortByFld = formatSortingField("SumVisits", "SUM(lang_visits)", ASG_USE_MYSQL)
	Case "language"	strAsgSortByFld = formatSortingField("lang_name", "lang_name", ASG_USE_MYSQL)
	Case else		strAsgSortByFld = formatSortingField("SumVisits", "SUM(lang_visits)", ASG_USE_MYSQL)
End Select
...
<div align="center">
	<div id="layout">
		<div id="layout_menutitle"><%= "<span class=""menusubtitle"">" & MENUGROUP_Visitors & " &raquo; " & MENUSECTION_VisitorSystems & " &raquo;</span> " & MENUSECTION_BrowsersLang %></div>
		<div id="layout_content">

<%

' :: Open tlayout :: MENUSECTION_BrowsersLang
Response.Write(builTableTlayout("", "open", MENUSECTION_BrowsersLang))

%>
<table width="100%" border="0" align="center" cellpadding="1" cellspacing="1">
  <tr>
	<td width="5%"  class="treport_title">&nbsp;</td>
	<td width="33%" class="treport_title"><%= TXT_browser_language %>
		<a href="?<%= strAsgAppend & "&amp;sortby=language&amp;sortorder=DESC" %>" title="<%= TXT_orderBy & "&nbsp;" & TXT_browser_language & "&nbsp;" & TXT_orderDesc %>">
		<img src="<%= STR_ASG_SKIN_PATH_IMAGE %>icons/order_desc.png" border="0" align="middle" alt="<%= TXT_orderBy & "&nbsp;" & TXT_browser_language & "&nbsp;" & TXT_orderDesc %>" /></a>
		<a href="?<%= strAsgAppend & "&amp;sortby=language&amp;sortorder=ASC" %>" title="<%= TXT_orderBy & "&nbsp;" & TXT_browser_language & "&nbsp;" & TXT_orderAsc %>">
		<img src="<%= STR_ASG_SKIN_PATH_IMAGE %>icons/order_asc.png" border="0" align="middle" alt="<%= TXT_orderBy & "&nbsp;" & TXT_browser_language & "&nbsp;" & TXT_orderAsc %>" /></a></td>
...
strAsgSQL = "SELECT lang_name, SUM(lang_hits) AS SumHits, SUM(lang_visits) As SumVisits " &_
	"FROM " & ASG_TABLE_PREFIX & "language "
' Month and Year
if strAsgMode = "month" then 
	strAsgSQL = strAsgSQL & "WHERE lang_period = '" & strAsgPeriod & "' "
	strAsgSQL = searchFor(strAsgSQL, false)		' Search
elseif strAsgMode = "all" then 
	strAsgSQL = searchFor(strAsgSQL, true)		' Search
end if
' Group information by following fields
strAsgSQL = strAsgSQL & " GROUP BY lang_name "
...
%>		  
  <tr <%= buildTableContRollover("treport_row") %> >
	<td class="treport_col" align="center"><img src="<%= STR_ASG_SKIN_PATH_IMAGE %>flags/<%= ShowIconLanguage(objAsgRs("lang_name")) %>" alt="<%= objAsgRs("lang_name") %>" align="middle" /></td>
	<td class="treport_col" align="left"><%= searchTerms(objAsgRs("lang_name"), "lang_name", asgSearchfor, asgSearchIn) %></td>
...
' :: Close tlayout :: MENUSECTION_BrowsersLang
...
Response.Write(builTableTlayout("x-rowSearch", "open", buildSwapDisplay("rowSearch", BARLABEL_DataSearch)))
	...
	' Row - Layers search
	Response.Write(buildLayerSearch("", "lang_name"))
